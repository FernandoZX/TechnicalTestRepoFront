...
folder are mapped to the application's root namespace, 'SubscriberPortal'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SubscriberPortal"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SubscriberPortal' in the same way
as `"SubscriberPortal/sass/src"`.
