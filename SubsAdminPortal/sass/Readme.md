...
folder are mapped to the application's root namespace, 'SubsAdminPortal'. This is set in
`"app.json"`:

    "sass": {
        "namespace": "SubsAdminPortal"
    }
...
included in the application's JavaScript code build. By default, files in this 
folder are mapped to the application's root namespace, 'SubsAdminPortal' in the same way
as `"SubsAdminPortal/sass/src"`.
